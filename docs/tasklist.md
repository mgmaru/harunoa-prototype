...
| 認証（8.1〜8.5） | 5 | [x] |
| 複数端末（9.1〜9.2） | 2 | [x] |
| オフライン対応（10.1〜10.4） | 4 | [x] |
| 非機能要件（11.1〜11.2） | 2 | [x] |
| レスポンシブ対応（12.1〜12.2） | 2 | [x] |

### 本番環境（P）

**実施日**: 2026-02-06
**環境**: https://harunoa-prototype.vercel.app（Vercel Production）
**テスト方針**: スモークテスト（主要フローのみ）

| カテゴリ | 件数 | チェック | 備考 |
|---------|:----:|:--------:|------|
| 認証（8.1, 8.4） | 2 | [x] | ログイン画面表示・ログアウト |
| プロジェクト管理（1.1） | 1 | [x] | プロジェクト作成 |
| 計測機能（2.1） | 1 | [x] | 計測開始〜停止の全フロー |
| 履歴管理（3.1） | 1 | [x] | 履歴表示・フィルタ |
| 集計・グラフ（4.1） | 1 | [x] | 期間別表示 |
| 複数端末（9.1） | 1 | [x] | データ同期 * |
| レスポンシブ対応（12.1） | 1 | [x] | スマホでの計測操作 |

\* 9.1は同一ブラウザでのビューポート変更による確認。手動テスト推奨
