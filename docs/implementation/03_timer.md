...
- [ ] 離脱時に確認ダイアログが表示
- [ ] ブラウザリロードで計測状態が復元される
- [ ] 計測中に別プロジェクトを開始しようとした場合、警告表示→確定操作の2ステップが必要
- [ ] プロジェクト切替時に現在の計測が自動停止し、セッションが確定保存される
- [ ] 同時に計測できるプロジェクトは1つのみである
- [ ] 集中モードでメモを入力できる
- [ ] メモ入力中も計測（経過時間カウント）が継続する
- [ ] タイマー操作がサーバに反映されない場合、エラー表示とリトライボタンが表示される
- [ ] リトライ3回失敗時に「オフラインモードで継続」が案内される

### レスポンシブ対応

- [ ] スマホ（Chrome）で計測開始/一時停止/再開/停止が操作できる
- [ ] スマホ（Chrome）で集中モードが画面全体に適切に表示される
- [ ] PC（Chrome）で各画面が適切に表示される
