...
- [ ] 手動再同期ができる
- [ ] ブラウザリロード後もキューが残っている
- [ ] オフライン中にページを閉じて再読み込みしても、計測継続中として復元される
- [ ] オンライン復帰時、キュー内のイベントが時刻付きで順番に送信される
- [ ] 複数端末から同時操作があった場合、タイムスタンプが新しい操作が採用される（後勝ち）
- [ ] 同期失敗時にリトライされ、上限がある

### 複数端末対応

- [ ] 端末Aで作成したプロジェクトが端末Bで表示される
- [ ] 端末Aで記録したセッションが端末Bの履歴に表示される
- [ ] 端末Aで変更した設定（通知ON/OFF等）が端末Bに反映される
