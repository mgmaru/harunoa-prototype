...
- [ ] プリセット選択でポモドーロが有効化される
- [ ] 「使用しない」選択でポモドーロが無効化される
- [ ] タイマー開始と同時にポモドーロの集中が開始される
- [ ] 集中時間終了で休憩に自動遷移する
- [ ] 休憩時間終了でIdle状態に戻る（自動繰り返しなし）
- [ ] 休憩中にスキップボタンでIdleに戻る
- [ ] タイマー停止でポモドーロも停止する
- [ ] プログレスバーが正しく進捗を表示する
- [ ] ブラウザリロード後もポモドーロ状態が復元される
- [ ] デフォルトプリセットが初回ログイン時に作成される
- [ ] 集中時間を24分に設定すると「集中時間は25分以上で設定してください」と表示され保存できない
- [ ] 休憩時間を4分に設定すると「休憩時間は5分以上で設定してください」と表示され保存できない
- [ ] 休憩時間を11分に設定すると「休憩時間は10分以内で設定してください...」と表示され保存できない
- [ ] 新しいポモドーロプリセットを作成できる
- [ ] プリセット名を変更できる
- [ ] プリセットを削除できる
- [ ] 利用中のプリセットを削除しようとすると「利用中のプリセットは削除できません...」と表示される
- [ ] プリセットで通知音（集中終了/休憩終了）を選択できる
- [ ] プリセット名が空で保存しようとすると「プリセット名を入力してください」と表示される

### エッジケース

- [ ] プリセット50件でも一覧が正しく表示される
