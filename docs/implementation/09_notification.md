...
- [ ] 通知音ON時に休憩終了で音が鳴る
- [ ] 通知音OFF時に音が鳴らない
- [ ] ブラウザ通知許可時にNotificationが表示される
- [ ] ブラウザ通知未許可時にアプリ内通知が表示される
- [ ] アプリ内通知が5秒後に自動消去される
- [ ] 通知バナーを手動で閉じられる
- [ ] 設定画面で通知音のON/OFFを切り替えられる
- [ ] 設定画面でブラウザ通知のON/OFFを切り替えられる
- [ ] ブラウザ通知をONにすると許可ダイアログが表示される
- [ ] プリセットで設定した通知音が使用される
- [ ] 「無音」を選択した場合は音が鳴らない
- [ ] 初期状態（初回ログイン時）で通知音がOFFになっている
- [ ] 初期状態（初回ログイン時）でブラウザ通知がONになっている
- [ ] ブラウザ通知がブロック済みの場合、ブラウザ設定からの変更を案内するメッセージが表示される
