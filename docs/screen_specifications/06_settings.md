...
| 2 | M011-02 | 警告表示 | 利用中プリセット警告 | 削除対象が利用中の場合のみ表示。デフォルトプリセットが自動で利用中になる旨を通知 | - |
| 3 | M011-03 | ボタン | キャンセルボタン | モーダルを閉じる | ○ |
| 4 | M011-04 | ボタン | 削除ボタン | プリセットを削除 | ○ |
...
| 3 | プリセット削除 | 削除確認後 | プリセットを削除。利用中プリセットの場合は削除後にデフォルトプリセットを自動で利用中に設定 |
