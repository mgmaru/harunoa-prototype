...
| 2 | M002-02 | カラーピッカー | 色選択 | 24色から選択、または自動割当。他プロジェクトが使用中の色にはチェックマークを表示（選択は可能） | - |
| 3 | M002-03 | チェックボックス | 自動割当 | 色を自動で割り当てる。未使用色を優先し、全色使用済みの場合は使用数が最も少ない色を選択する（アクティブなプロジェクトのみで判定） | - |
...
| 2 | M003-02 | カラーピッカー | 色選択 | 24色から選択。編集対象以外のプロジェクトが使用中の色にはチェックマークを表示（選択は可能） | ○ |
