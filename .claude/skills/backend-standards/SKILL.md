...
description: |
  HaruNoaプロジェクトのバックエンドコーディング規約を提供。Firebase/Firestore操作、services層設計、セキュリティルールを適用する。
  Use when: (1) Firebase/Firestoreを操作する時、(2) src/services/配下のファイルを編集する時、(3) Server Actions/Route Handlersを書く時、(4) データアクセスロジックを実装する時、(5) 認証・認可処理を実装する時。
---
...
export const getProjects = async (userId: string): Promise<Project[]> => {
  ...
  return snapshot.docs.map(doc => ({
    ...doc.data(),
    id: doc.id,
    createdAt: (doc.data().createdAt as Timestamp).toDate(), // ここで変換
  })) as Project[]
}
```
